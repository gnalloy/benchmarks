...
## 当前版本 TLS 1.1/1.2 全矩阵复验

2026-09-04 使用 benchmark `6e0d2b754350e63b2028a676494432f6e0d9b4e9` 的当前统一二进制，分别执行饱和和 `70,000 ops/s` 两套矩阵。每套包含 TLS 1.1/1.2、128B/1KiB、四个适用框架各五轮，共 80 个有效结果；两套均全部 `errors=0`。gnet 和 netpoll 没有框架自带 HTTP codec，40 个组合逐项记录为 N/A，没有引入 benchmark 自写解析。

### 当前饱和吞吐与 P99

| TLS | Payload | 服务端 | 吞吐中位数 ops/s | RTT P99 中位数 ms |
| --- | ---: | --- | ---: | ---: |
| 1.1 | 128B | Gnalloy | 104,428.95 | **1.244** |
| 1.1 | 128B | Hertz | 102,790.16 | 2.654 |
| 1.1 | 128B | fasthttp | **108,626.23** | 3.931 |
| 1.1 | 128B | Netty | 79,951.75 | 5.313 |
| 1.1 | 1KiB | Gnalloy | 91,337.79 | **2.229** |
| 1.1 | 1KiB | Hertz | 91,668.08 | 3.629 |
| 1.1 | 1KiB | fasthttp | **93,190.96** | 3.311 |
| 1.1 | 1KiB | Netty | 68,933.45 | 5.668 |
| 1.2 | 128B | Gnalloy | 133,264.38 | **1.204** |
| 1.2 | 128B | Hertz | 125,950.28 | 9.735 |
| 1.2 | 128B | fasthttp | **134,065.77** | 7.821 |
| 1.2 | 128B | Netty | 93,172.61 | 4.887 |
| 1.2 | 1KiB | Gnalloy | **99,300.56** | 1.530 |
| 1.2 | 1KiB | Hertz | 96,520.43 | **1.255** |
| 1.2 | 1KiB | fasthttp | 98,645.10 | 2.517 |
| 1.2 | 1KiB | Netty | 74,616.26 | 5.215 |

饱和 P99受容量差异和单轮系统长尾影响较大，主要容量判断使用吞吐中位数。Gnalloy 在 TLS 1.1 的两个 payload 和 TLS 1.2/128B 均未超过 fasthttp 吞吐；TLS 1.2/1KiB 吞吐领先全部适用对手。饱和 P99除 TLS 1.2/1KiB 被 Hertz 领先外，其余三个组合 Gnalloy最低。

### 当前 70k offered-rate RTT

| TLS | Payload | 服务端 | 实际吞吐中位数 ops/s | RTT P50 中位数 ms | RTT P99 中位数 ms |
| --- | ---: | --- | ---: | ---: | ---: |
| 1.1 | 128B | Gnalloy | 69,990.11 | 0.457 | 1.056 |
| 1.1 | 128B | Hertz | 69,991.87 | 0.460 | 1.244 |
| 1.1 | 128B | fasthttp | **69,993.29** | **0.454** | **1.002** |
| 1.1 | 128B | Netty | 69,992.17 | 0.550 | 5.401 |
| 1.1 | 1KiB | Gnalloy | 69,990.11 | **0.546** | 1.219 |
| 1.1 | 1KiB | Hertz | **69,990.65** | 0.549 | **1.196** |
| 1.1 | 1KiB | fasthttp | 69,989.97 | 0.550 | 1.237 |
| 1.1 | 1KiB | Netty | 69,901.55 | 0.672 | 4.840 |
| 1.2 | 128B | Gnalloy | 69,993.17 | **0.378** | **0.821** |
| 1.2 | 128B | Hertz | 69,993.44 | 0.388 | 0.937 |
| 1.2 | 128B | fasthttp | **69,993.57** | 0.381 | 0.821 |
| 1.2 | 128B | Netty | 69,992.30 | 0.452 | 5.234 |
| 1.2 | 1KiB | Gnalloy | **69,993.60** | **0.456** | **0.964** |
| 1.2 | 1KiB | Hertz | 69,991.94 | 0.522 | 1.012 |
| 1.2 | 1KiB | fasthttp | 69,991.16 | 0.483 | 1.065 |
| 1.2 | 1KiB | Netty | 69,991.93 | 0.546 | 5.074 |

TLS 1.1/128B 的 Gnalloy RTT P99 比 fasthttp 高 `5.40%`；TLS 1.1/1KiB 比 Hertz 高 `1.95%`。TLS 1.2/128B 与 fasthttp 的 P99仅相差约 `0.0004ms`，处于测量噪声级别；TLS 1.2/1KiB 的 Gnalloy RTT P50/P99 均最低。当前证据不支持 TLS 1.1/1.2 全矩阵全面领先的表述。
