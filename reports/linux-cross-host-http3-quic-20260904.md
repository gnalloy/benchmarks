...
### 当前版本 QUIC stream 饱和 P99 复验

2026-09-04 使用 benchmark `6e0d2b754350e63b2028a676494432f6e0d9b4e9` 的统一二进制重新执行 128B/1KiB 各五轮。Gnalloy 服务端和公共客户端二进制 SHA-256 均为 `b64a25fd8e4b88cf07714ddec8b5966fc79fe5b06f6a304876f85c0cdd2b4b7a`，Netty JAR 为 `7072cfcc956cc6b72b6dc46c3b9fcacd21249fc532d8bd6c088fb8b97eff9e16`。共 20 个 case，全部 `errors=0`。

| Payload | 服务端 | 吞吐中位数 ops/s | RTT P50 中位数 ms | RTT P95 中位数 ms | RTT P99 中位数 ms | RTT P99.9 中位数 ms | RTT P99 范围 ms |
| ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 128B | Gnalloy | **70,073.66** | **0.764** | **1.951** | 2.830 | 5.071 | 2.689-3.152 |
| 128B | Netty | 40,188.02 | 1.397 | 2.382 | **2.685** | **3.828** | 2.685-2.874 |
| 1KiB | Gnalloy | **64,152.70** | **0.845** | **2.002** | 3.060 | **5.235** | 2.906-3.340 |
| 1KiB | Netty | 40,620.59 | 1.452 | 2.565 | **2.929** | 7.865 | 2.874-3.090 |

Gnalloy 饱和吞吐在 128B 和 1KiB 分别领先 `74.36%` 和 `57.93%`，RTT P50/P95 同时更低；饱和 RTT P99分别高 `5.37%` 和 `4.49%`。由于 Gnalloy 在更高实际吞吐下运行，该 P99差值描述各自饱和点，不能替代相同 offered-rate 的延迟对比。当前版本仍未达到 QUIC 饱和 P99 全面领先目标。
