...
2026-09-04 使用 benchmark `6e0d2b754350e63b2028a676494432f6e0d9b4e9` 的统一二进制重新执行固定 `60,000 ops/s` 低负载矩阵。服务端保持 2 个 I/O loop，Gnalloy 使用 1 个 boss 和 2 个物理核 worker，服务端 `GOMAXPROCS=3`；每个 payload、每个框架执行 10 轮并轮转顺序。共 60 个 case，全部 `errors=0`。

| Payload | Framework | Throughput median ops/s | Total P99 median ms | Total P99 range ms | Schedule P99 median ms | RTT P99 median ms | RTT P99 range ms |
| ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 128B | Gnalloy | 59,998.77 | 0.253 | 0.224-0.458 | 0.058 | 0.198 | 0.188-0.233 |
| 128B | gnet | 59,998.90 | **0.231** | 0.223-0.285 | 0.055 | **0.196** | 0.186-0.259 |
| 128B | Netty | 59,998.96 | 0.241 | 0.226-0.361 | 0.056 | 0.220 | 0.189-0.303 |
| 1KiB | Gnalloy | 59,998.63 | 0.416 | 0.268-2.883 | 0.062 | 0.307 | 0.231-0.573 |
| 1KiB | gnet | 59,998.66 | **0.286** | 0.266-2.108 | 0.056 | **0.266** | 0.230-1.340 |
| 1KiB | Netty | 59,998.66 | 0.323 | 0.262-2.488 | 0.059 | 0.274 | 0.228-0.573 |

128B 的 Gnalloy RTT P99 中位数比 gnet 高 `0.89%`，比 Netty 低 `9.88%`，三者仍处于接近区间。1KiB 的 Gnalloy RTT P99 中位数比 gnet 高 `15.06%`，比 Netty 高 `11.81%`，当前版本未达到低负载尾延迟领先目标。各框架均存在少量单轮尖峰，表中同时保留范围，避免用单轮最好值替代分布。
