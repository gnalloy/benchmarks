[CmdletBinding()]
param(
    [ValidateSet("http3", "quic-stream")]
    [string]$Protocol = "http3",
...
$protocolDirectory = if ($Protocol -eq "http3") { "http3" } else { "quic-stream" }
$protocolPort = if ($Protocol -eq "http3") { 19093 } else { 19094 }
$alpn = if ($Protocol -eq "http3") { "h3" } else { "gnalloy-quic" }
if ([string]::IsNullOrWhiteSpace($ServerRepo)) {
    $ServerRepo = "/opt/test/gnalloy/${protocolDirectory}-server-20260904/benchmarks"
}
if ([string]::IsNullOrWhiteSpace($ServerBindAddress)) {
    $ServerBindAddress = "0.0.0.0:$protocolPort"
}
if ([string]::IsNullOrWhiteSpace($TargetAddress)) {
    $TargetAddress = "172.16.8.171:$protocolPort"
}
if ([string]::IsNullOrWhiteSpace($GnalloyBench)) {
    $GnalloyBench = "/opt/test/gnalloy/${protocolDirectory}-server-20260904/gnalloy-bench"
}
if ([string]::IsNullOrWhiteSpace($NettyBenchJar)) {
    $NettyBenchJar = "/opt/test/gnalloy/${protocolDirectory}-server-20260904/netty-bench.jar"
}
if ([string]::IsNullOrWhiteSpace($ClientBench)) {
    $ClientBench = "/opt/test/gnalloy/${protocolDirectory}-client-20260904/gnalloy-bench"
}
if ([string]::IsNullOrWhiteSpace($Output)) {
    $mode = if ($TargetRate -gt 0) { "rate-$TargetRate" } else { "saturation" }
    $Output = Join-Path $repoRoot "reports/raw/linux-cross-host-$protocolDirectory-$mode.out"
}
$remotePIDFile = "/tmp/gnalloy-$protocolDirectory-cross-host.pid"
$remoteLog = "/tmp/gnalloy-$protocolDirectory-cross-host.log"
...
cd '__SERVER_REPO__'
: >'__LOG__'
nohup env PROTOCOL='__PROTOCOL__' PAYLOAD='__PAYLOAD__' SERVER_ADDR='__BIND_ADDR__' SERVER_CPU_SET='__SERVER_CPUS__' SERVER_GOMAXPROCS='__SERVER_PROCS__' EVENT_LOOPS='__EVENT_LOOPS__' SERVER_PID_FILE='__PID_FILE__' GNALLOY_BENCH='__GNALLOY_BENCH__' NETTY_BENCH_JAR='__NETTY_JAR__' bash ./scripts/run-linux-quic-server.sh '__FRAMEWORK__' >'__LOG__' 2>&1 </dev/null &
'@
...
$serverCheck = "test -f '$ServerRepo/scripts/run-linux-quic-server.sh' && test -x '$GnalloyBench' && test -f '$NettyBenchJar' && test -x /opt/software/java21/bin/java && taskset -c '$ServerCPUSet' true && printf ready"
...
try {
    ...
    Set-Content -LiteralPath $Output -Value @(
        ...
        "protocol=$Protocol tlsVersion=1.3 alpn=$alpn payloads=$($Payloads -join ',') connections=$Connections messages=$Messages warmupMessages=$WarmupMessages latencySampleRate=$LatencySampleRate targetRate=$TargetRate repetitions=$Repetitions cooldownSeconds=$CooldownSeconds",
        "serverCPUSet=$ServerCPUSet serverGOMAXPROCS=$ServerGoMaxProcs clientCPUSet=$ClientCPUSet clientGOMAXPROCS=$ClientGoMaxProcs commonClient=gnalloy-transport-quic+application-codec performanceGovernor=$SetPerformanceGovernor",
        "unsupportedFrameworks=hertz,gnet,netpoll,fasthttp status=N/A reason=no-native-$protocolDirectory-codec"
    ) -Encoding utf8
    ...
    foreach ($payload in $Payloads) {
        foreach ($unsupported in $unsupportedFrameworks) {
            if ($Frameworks -contains $unsupported) {
                Add-Content -LiteralPath $Output -Value "case=$unsupported protocol=$Protocol tlsVersion=1.3 payload=$payload status=N/A reason=no-native-$protocolDirectory-codec" -Encoding utf8
            }
        }
        for ($run = 1; $run -le $Repetitions; $run++) {
            foreach ($framework in (Get-RotatedFrameworks -Run $run)) {
                ...
                $case = "case=$framework protocol=$Protocol tlsVersion=1.3 payload=$payload run=$run"
                Write-Output $case
