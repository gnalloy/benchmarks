...
case "${FRAMEWORK}" in
  gnalloy)
    ...
    exec taskset -c "${SERVER_CPU_SET}" env GOMAXPROCS="${SERVER_GOMAXPROCS}" "${GNALLOY_BENCH}" \
      -protocol http3 -server-only=true -addr "${SERVER_ADDR}" -payload "${PAYLOAD}" \
      -tls-version 1.3 -alpn h3 -timeout 5m
    ;;
  netty)
    ...
    exec taskset -c "${SERVER_CPU_SET}" "${JAVA_BIN}" -XX:ActiveProcessorCount="${SERVER_GOMAXPROCS}" \
      -jar "${NETTY_BENCH_JAR}" --protocol http3 --server-only true --addr "${SERVER_ADDR}" \
